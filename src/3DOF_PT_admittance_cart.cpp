...
Admittance_Status_Cart getControllOutput(const Eigen::Matrix<double,6,6>& M, const Eigen::Matrix<double,6,6>& B, const Eigen::Matrix<double,6,6>& K,
    ...
    for(int i=0;i<6;i++)
    {
        ...
        ctrlStatus.speed[i] = targetStatus.speed[i] - Xe_vel;
...
int main(int argc, char** argv) {
    ...
    //ros初始化
    ros::init(argc, argv, "3DOF_PT_admittance_cart");
    ros::NodeHandle n;
...
//    actualStatus.speed = ctrlStatus.speed;
        ...
        //期望位姿为初始位姿，期望速度与期望外力均为0
        pt.targetStatus.pose = targetPose;
        pt.targetStatus.speed = Eigen::MatrixXd::Zero(6,1);
        ...
        */
        ...
        tragetPosition = pt.ctrlStatus.pose.block<3,1>(0,0) + pt.targetStatus.pose.block<3,1>(3,0);  
        tragetRotationMatrix = RPY_rotationMatrix(pt.ctrlStatus.pose.block<3,1>(3,0)) * quaternion_rotationMatrix(rotationVector_quaternion(pt.targetStatus.block<3,1>(3,0)));
