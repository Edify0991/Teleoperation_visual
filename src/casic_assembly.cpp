...
int main(int argc, char **argv)
{
  ...
  Process_FT_Data KW_FT(n,"/ft_data","/processed_ft_data",CASIC_);
  ...
  assemblyPose << -0.10167-0.005, 0.65228+0.005, 0.19793, 0.0429, 3.1324, -0.0284;
  ...
  ctrllerArgs.mode = mode1;
  ctrllerArgs.poseCtrlArgsInit(ctrllerArgs.admSysArgs);

  bool assemblyFinishFlag = false;

  //初始化完成，进入主循环
  while(ros::ok() & !assemblyFinishFlag)
  {
    ctrllerArgs.targetStatus.pose = trajectory.getNextPlanPose();
//        ctrllerArgs.actualStatus.pose = ctrllerArgs.ctrlStatus.pose;

//    std::cout << "ctrllerArgs.targetStatus.pose  " << ctrllerArgs.targetStatus.pose.transpose() << std::endl;
    ...
    std::cout << "ctrllerArgs.actualStatus.force  " << ctrllerArgs.actualStatus.force.transpose() << std::endl;
...
//    ctrllerArgs.modeSwitch_DI(URData.getDigitalInput());
    ...
    if(ctrllerArgs.actualStatus.speed[2] > 0.0 && ctrllerArgs.actualStatus.pose[2] < assemblyPose[2] - 0.035)
    {
    ...
    {
      if(ctrllerArgs.actualStatus.pose[2] > assemblyPose[2] +0.015){
